...
p_c_municipio VARCHAR(7), p_id_pais INT, p_id_sesion_registro INT)
BEGIN
	-- Validando Tipos
	IF tipo = 1
	THEN
...
BEGIN
    ...
    DECLARE	edad_registro INT;
    -- Si es el año actual
    IF DATE_FORMAT(CURRENT_TIMESTAMP(), '%Y') = anio_registro
    THEN
		SELECT	TIMESTAMPDIFF(YEAR, fecha_nac, CURRENT_TIMESTAMP())
        INTO	edad_registro;
	ELSE
		SELECT	TIMESTAMPDIFF(YEAR, fecha_nac, CONCAT(@anio_registro,'-12-31'))
        INTO	edad_registro;
    END IF;
    ...
    THEN
        ...
        INTO	v_id_seccion
        FROM	seccion
        WHERE	edad_registro BETWEEN edad_inicial AND edad_final
